...
void telemetryInit(TelemetryConfig_t* info)
{
  ...
  s.itemCount = 0;
  s.dataTable = malloc(sizeof(TDataVar_t*) * info->valueTableCount);
  memset(s.dataTable, 0, sizeof(TDataVar_t*) * info->valueTableCount);

  s.valueModCount = 0;
  s.valueMods = malloc(sizeof(TDataFramePacket_t) * info->valueModBufferCount);
  memset(s.valueMods, 0, sizeof(TDataFramePacket_t) * info->valueModBufferCount);
  ...
  tdv_telemetry_update_us.value.u32 = timer_hw->timelr;
...
void telemetry_sample(TDataVar_t* dataVar)
{
  ...
  tdv_telemetry_queue.value.u32 = s.valueModCount;
...
void __time_critical_func(telemetry_send)(uint64_t start, uint64_t now)
{
  ...
  if( tdv_next_desc_send.value.u32 < s.itemCount)
  {
    uint8_t* descStart = ((uint8_t*)s.valueMods) +sendLength;

    

    TDataVar_t* var = value_table_get(tdv_next_desc_send.value.u32+1);
    // printf("--- %s --- 1\n", var->meta.name);

    sendLength += telemetry_write_desc_frame((TDataDescFramePacket_t*)descStart, var );
    ++tdv_next_desc_send.value.u32;

    // printf("--- %s --- 2\n", var->meta.name);
  }
  else
  {
    tdv_next_desc_send.value.u32 = 0;
  }
  ...
  uint32_t delta = timer_hw->timelr - tdv_telemetry_update_us.value.u32;
  tdv_telemetry_update_us.value.u32 = delta;
}
