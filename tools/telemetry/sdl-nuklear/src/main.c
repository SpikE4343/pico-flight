...
int main(void)
{
  ...
  while (running)
  {
    ...
    if (nk_begin(ctx, "Telemetry", nk_rect(350, 50, 230, 250),
                 NK_WINDOW_BORDER | NK_WINDOW_MOVABLE | NK_WINDOW_SCALABLE |
                     NK_WINDOW_MINIMIZABLE | NK_WINDOW_TITLE))
    {
      float ratio[] = {0.05f, 0.50f, 0.20f};
      for (int id = 1; id < tdv_telemetry_val_count.v.u32; ++id)
      {
        ...
        nk_labelf(ctx, NK_TEXT_RIGHT, "%u", id);

        nk_labelf(ctx, NK_TEXT_LEFT, "%s", v->meta.name);
        ...
        case Tdt_u32:
        case Tdt_u16:
          nk_property_int(ctx, "", 0, &v->v.u32, 0, 0, 0);
          break;

        case Tdt_i8:
        case Tdt_i16:
        case Tdt_i32:
          nk_property_int(ctx, "", 0, &v->v.i32, 0, 0, 0);
          break;

        case Tdt_b8:
          nk_checkbox_label(ctx, "", &v->v.b8);
          break;
        case Tdt_c8:
          break;

        case Tdt_f32:
          nk_property_float(ctx, "", 0, &v->v.f32, 0, 0, 0);
          break;
        ...
        if (nk_input_is_mouse_hovering_rect(&ctx->input, bounds))
          nk_tooltipf(ctx, "%s", v->meta.desc);
        // nk_labelf(ctx, NK_TEXT_LEFT, "%s", v->meta.desc);
      }
